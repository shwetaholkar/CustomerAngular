...
export class ChatBarComponent extends AppComponentBase implements OnInit, AfterViewInit {
    ...
    remove(user: FriendDto): void {
        this.message.confirm(this.l("AreYouSureToRemoveFromFriends"), null,
            (isConfirmed) => {
                if (isConfirmed) {
                    this._friendshipService.removeFriend(user.friendUserId, user.friendTenantId ? user.friendTenantId : undefined).subscribe(() => {
